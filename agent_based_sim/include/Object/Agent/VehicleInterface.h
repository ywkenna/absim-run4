...
public:
    ...
    VehicleInterface(Route* _route, string _departTime, int _routeIndex,  ID _ID, VehicleClass _class, VehicleType _type);
//    VehicleInterface(map<RouteSelect,Route*> _routes, string _departTime, int _routeIndex,  ID _ID, VehicleClass _class, VehicleType _type);
	...
	int getMyLinkIndex() const {return myLinkIndex;	}
	string getRouteString() const;
	int getNextOffsetOfLocalRoute() const;
    Route* getMyRoute() {  return myRoute; }
...
//	Link* getNextLink(Link* link);
	...
	Result fixMyLinkIndex(Link* _link);
...
//    void clearPassingInfo(SALTTime currentStep, int periodLength);
    ...
    map<RouteSelect,string> getMyCandidateRoutes() {
        return this->myCandidateRoutes;
    }

    string getMyCandidateRouteWithPreference(RouteSelect _rtalgorithm);

    SALT::Result setMyCandidateRoutes(RouteSelect _rtalgorithm, string _routeString);

    void cleanMyCandidateRoutes(){
        this->myCandidateRoutes.clear();
    }

    void setMyProperty(RouteSelect _rtalgorithm) {
        this->mySelectPref= _rtalgorithm;
    }

    map<int,map<int,tuple<SALTTime,SALTTime,SALTTime>>> getMyPassingInfo(){
        return this->myPassingInfo;
    }

    SALT::Result replaceCurrentRoutesWithPreference(NetworkManager *networkManagerToRef);
...
protected:
    ...
    VehicleType myType = COMMON;  // VehicleType: Common or Special (For dealing with the exceptional event)
	string myShape="";
...
//	tuple<CellInterface*, CellInterface*> myNextValidCellCache; // <current cell, next valid cell>
    ...
    // For Version4
    map<RouteSelect,string> myCandidateRoutes;     // Multiple Routes -->  Agent Behavior Model choose 1 route
    RouteSelect mySelectPref = MINTRVTIME; // RouteSelect: MINTRVTIME, MINDIST, WIDEROAD
};
