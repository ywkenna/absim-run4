...
public:
    ...
    Result recordVehicleInfo(VehicleInterface* veh);
    Result refreshTrajectoryMap();
    ...
    map<ID, string> getTotalVehiclesRoute();

    Result completeRecordTrajectory();
    Result writeTrajectoryInfo(SALTTime curStep);
    void setWirteRecord() {
        isWriteRecord = true;
    }
    ...
    string printVehicleFinalRouteInfo();
    string printVehiclesPassingInfo();
...
private:
    ...
    map<ID, string> myVehiclesRouteInfo;
    map<ID, map<ID, tuple<int, SALTTime, SALTTime>>> myVehiclesTrajectory;


    RouteSelect myRoutePref=MINTRVTIME;    // Temporarily, define in this class
    string myOutputDir;
    ...
    string myPeriodicOutputHeader = "vehID,linkID,seq,enterTime,leaveTime\n";    // for V3
    string myPeriodicOutputBuffer="";

    std::ofstream* myPeriodicOutputFileStream=NULL;

    SALTTime myBeginTime;
    SALTTime myEndTime;
